...
上流へ送る前に接続を終了し、再接続で選び直す。中継側からの再送や接続内の認証差し替えはしない。
確認するのは最大14バイトのフレームヘッダーだけで、本文・圧縮データはストリームのまま通す。
Codex 自身の再接続による回復は実機では未検証。
専用RR URLのWebSocketは接続ごとに巡回し、接続内は同じアカウントを使う。

認証ヘッダーを選択アカウントの Bearer と Account ID に替える。本文を再生成せず、
...
通常のfill-firstから除外するのは、明示設定したRR URLと管理用 `/_pool/status` だけ。
...
Python の10テストに Local Capture・OSの信頼変更・実アカウント通信は含まれない。
